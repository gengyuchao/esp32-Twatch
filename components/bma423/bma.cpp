...
bma423_t *BMA::_bus = nullptr;

BMA::BMA(bma423_t *bus)
{
    _bus = bus;
    _init = false;
}
...
{
    return _bus->read(_bus->handle,addr, reg, data, len);
}

uint16_t BMA::write(uint8_t addr, uint8_t reg, uint8_t *data, uint16_t len)
{
    return _bus->write(_bus->handle, addr, reg, data, len);
}
...
{
    uint8_t reg = 0xB6;
    write(BMA4_I2C_ADDR_SECONDARY, 0x7E, &reg, 1);
}
